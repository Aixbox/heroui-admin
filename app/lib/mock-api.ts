...
export interface DashboardStats { revenue: string; orders: number; customers: number; conversion: string; }
export interface MockApiError { message: string; fieldErrors?: Record<string, string>; }

const getMockApiBaseUrl = () => typeof window !== "undefined" ? "" : process.env.MOCK_API_URL ?? "http://localhost:8787";

async function mockRequest<T>(path: string, init?: RequestInit): Promise<T> {
  const response = await fetch(`${getMockApiBaseUrl()}${path}`, { ...init, credentials: "include", headers: { "Content-Type": "application/json", ...init?.headers } });
  if (!response.ok) {
...
export const mockApi = {
  ...
  login: (email: string, password: string) => mockRequest<{ user: User }>("/mock-api/auth/login", { method: "POST", body: JSON.stringify({ email, password }) }),
